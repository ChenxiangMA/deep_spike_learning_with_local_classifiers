    ...
    def forward(self, x, y, y_onehot, time_window=10):
        ...
        for step in range(time_window):
            ...
            if self.first_layer:
                ...
                h = self.encoder(x[step]) # Input current
            ...
            if step == 0:
                ...
                if self.training or self.is_print_stats:
                    # y_hat_mem_aux and y_hat_spike_aux are auxiliary for generating loss.
                    y_hat_mem_aux = self.decoder_y(spike)
                    y_hat_spike_aux = ExponentialSurroGrad.apply(y_hat_mem_aux, self.thresh)
            else:
                mem = mem * self.decay + h - spike * self.thresh * self.decay
                spike = ExponentialSurroGrad.apply(mem, self.thresh)
                # Store output spikes to be propagated to downstream neurons
                spike_return = torch.cat((spike_return, spike), dim=0)
                if self.training or self.is_print_stats:
                    y_hat_mem_aux = y_hat_mem * self.decay + self.decoder_y(spike) - y_hat_spike * self.thresh * self.decay
                    y_hat_spike_aux = ExponentialSurroGrad.apply(y_hat_mem_aux, self.thresh)
            if self.training or self.is_print_stats:
                ...
                if self.training:
                    ...
                    self.optimizer.zero_grad()
                    # retain graph such that it can be used at the next time step
                ...
                if step == 0:
                    y_hat_mem = self.decoder_y(spike.detach())
                    y_hat_spike = ExponentialSurroGrad.apply(y_hat_mem, self.thresh)
                    spike_sum = y_hat_spike
                else:
                    y_hat_mem = y_hat_mem * self.decay + self.decoder_y(spike.detach()) - y_hat_spike * self.thresh * self.decay
                    y_hat_spike = ExponentialSurroGrad.apply(y_hat_mem, self.thresh)
