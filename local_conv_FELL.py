            ...
            if step == 0:
                ...
                if self.training or self.is_print_stats:
                    if self.avg_pool is not None:
                        mem_pool = self.avg_pool(spike) # Average pooling operation is applied first in order to reduce the number of auxilinary weights
                    else:
                        mem_pool = spike
                    y_hat_mem_aux = self.decoder_y(mem_pool.view(mem_pool.size(0), -1))
                    y_hat_spike_aux = ExponentialSurroGrad.apply(y_hat_mem_aux, self.thresh)
            else:
                ...
                if self.training or self.is_print_stats:
                    if self.avg_pool is not None:
                        ...
                        mem_pool = spike
                    y_hat_mem_aux = y_hat_mem * self.decay + self.decoder_y(mem_pool.view(mem_pool.size(0), -1)) - y_hat_spike * self.thresh * self.decay
                    y_hat_spike_aux = ExponentialSurroGrad.apply(y_hat_mem_aux, self.thresh)
        
            if self.training or self.is_print_stats:
                loss_sup = F.mse_loss(y_hat_spike_aux, y_onehot.detach())
                if step == 0:
                    loss = loss_sup
                else:
                    loss = loss + loss_sup
                ...
                if self.training:
                    self.optimizer.zero_grad()
                    loss_sup.backward(retain_graph=True) # Retain graph for gradients at the next time step
                    # self.optimizer.step() # Update all parameters

                # Remove unneeded parts of computational graph
                mem_pool = None
                y_hat_mem_aux = None
                y_hat_spike_aux = None

                if self.avg_pool is not None:
                    mem_pool = self.avg_pool(spike.detach())
                else:
                    mem_pool = spike.detach()
                ...
                if step == 0:
                    ...
                    y_hat_mem = y_hat_mem * self.decay + self.decoder_y(mem_pool.view(mem_pool.size(0), -1)) - y_hat_spike * self.thresh * self.decay
                    y_hat_spike = ExponentialSurroGrad.apply(y_hat_mem, self.thresh)
